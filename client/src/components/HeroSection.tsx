 ...
 * Hero preservando métricas originais, mas ajustando a narrativa para Pedro Andreoli
 */
...
const highlights = [
  ...
  { icon: Building2, label: "Visão Empreendedora", color: "text-primary" },
];
  ...
  return (
    ...
    >
      <div className="absolute inset-0 z-0">
        <img src={HERO_BG} alt="Background" className="w-full h-full object-cover opacity-30" />
      ...
      <div className="container relative z-10 py-20">
        <div className="grid lg:grid-cols-5 gap-12 items-center">
          <motion.div ref={ref} variants={containerVariants} initial="hidden" animate="visible" className="lg:col-span-3 space-y-8">
            ...
            <motion.div variants={itemVariants}>
              <p className="text-sm font-mono text-muted-foreground mb-2 tracking-widest uppercase">
                Estudante de ADS na PUC-GO | Fundador de Software
              </p>
            ...
            <motion.div variants={itemVariants} className="relative pl-5 border-l-2 border-accent/60">
              <p className="text-lg text-foreground/90 leading-relaxed font-medium">
                Meu nome é Pedro Nascimento Andreoli, sou apaixonado por tecnologia e garanto que <span className="text-accent font-semibold">você não vai se arrepender de me contratar.</span>
              </p>
              <p className="mt-4 text-base text-muted-foreground leading-relaxed">
                Além de cursar ADS na PUC-GO, foco em criar sistemas que geram <span className="text-foreground font-semibold">vendas e economia de tempo.</span> Se você quer alguém que desenvolva com a mentalidade de dono, focado em resultados reais, eu sou essa pessoa.
              </p>
            </motion.div>

            <motion.div variants={itemVariants} className="flex flex-wrap gap-3">
              {highlights.map(({ icon: Icon, label, color }) => (
                <div key={label} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-secondary/60 border border-border text-sm font-medium">
            ...
            <motion.div variants={itemVariants} className="flex flex-wrap gap-4">
              <a href="#projetos" className="flex items-center gap-2 px-6 py-3 bg-primary text-primary-foreground font-semibold rounded-lg hover:bg-primary/90 transition-all duration-200">
                Ver Projetos
                <ArrowRight className="w-4 h-4" />
              </a>
              <a href="#contato" className="flex items-center gap-2 px-6 py-3 border border-border text-foreground font-semibold rounded-lg hover:bg-secondary/60 transition-all duration-200">
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.4 }} className="lg:col-span-2 space-y-4">
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.9 }} className="p-5 rounded-xl bg-card/60 border border-border backdrop-blur-sm">
              <p className="text-xs font-mono text-muted-foreground mb-3 uppercase tracking-widest">Stack Principal</p>
              <div className="flex flex-wrap gap-2">
                {["Python", "React", "Node.js", "TypeScript", "PostgreSQL", "n8n"].map((tech) => (
                  <span key={tech} className="px-2.5 py-1 text-xs font-mono bg-primary/10 text-primary border border-primary/20 rounded-md">
                    {tech}
                  </span>
                ))}
              </div>
